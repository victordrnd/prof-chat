...
export class Init1655024463572 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`messages\` (\`id\` int unsigned NOT NULL AUTO_INCREMENT, \`type\` varchar(255) NOT NULL, \`content\` varchar(255) NOT NULL, \`created_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` timestamp(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`roomId\` int unsigned NULL, \`userId\` int unsigned NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`rooms\` (\`id\` int unsigned NOT NULL AUTO_INCREMENT, \`name\` varchar(255) NOT NULL, PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`users_rooms_rooms\` (\`usersId\` int unsigned NOT NULL, \`roomsId\` int unsigned NOT NULL, INDEX \`IDX_be046c829cc9f45adfe322e75e\` (\`usersId\`), INDEX \`IDX_df57dc27d23e464abaa467017a\` (\`roomsId\`), PRIMARY KEY (\`usersId\`, \`roomsId\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`rooms_users_users\` (\`roomsId\` int unsigned NOT NULL, \`usersId\` int unsigned NOT NULL, INDEX \`IDX_cbe951142bc45a33a744256516\` (\`roomsId\`), INDEX \`IDX_6b3c5f4bbfb29a84a57e442af5\` (\`usersId\`), PRIMARY KEY (\`roomsId\`, \`usersId\`)) ENGINE=InnoDB`);
